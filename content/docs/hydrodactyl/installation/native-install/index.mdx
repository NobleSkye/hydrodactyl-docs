...
This process is also quite straight-forward.
  ...
  ```bash
  curl -Lo panel.tar.gz https://github.com/blueprintframework/hydrodactyl/releases/latest/download/panel.tar.gz
...
### If using MariaDB
  ...
  ```sql
  CREATE USER 'hydrodactyl'@'127.0.0.1' IDENTIFIED BY 'yourPassword';
...
## Installing the panel
  ...
  <Callout type="warn">
    ...
    the first time and do not have any Hydrodactyl Panel data in the database.
  </Callout>
  ```bash
  ...
  ```bash
    grep APP_KEY /var/www/hydrodactyl/.env
  ```
  Save this somewhere safe, as you'll need it to recover data should that need arise.

### Environment Configuration
Hydrodactyl's core environment is easily configured using a few different CLI commands built into the app. This step will cover setting up things such as sessions, caching, database credentials, and email sending.
  ```bash
...
### Set Permissions
  ```bash
    ...
    chown -R nginx:nginx /var/www/hydrodactyl/*

    # If using Apache on RHEL / Rocky Linux / AlmaLinux
    chown -R apache:apache /var/www/hydrodactyl/*
...
The first thing we need to do is create a new cronjob that runs every minute to process specific Hydrodactyl tasks,
...
### Telemetry
  Hydrodactyl Collects No data what-so ever. This might change to anonymous in the future to facilitate
