...
### Local Backups
By default, Hydrodactyl Panel uses local storage via Elytra for backups. That said, this method of backup storage can be explicitly set with the following configuration in your environment(.env):
```dotenv
...
Due to changes in the ecosystem from Hydrodactyl to Hydrodactyl, Multipart uploads for buckets no longer
happen on the panel side. Instead, it's fully managed by your nodes.

## Reverse Proxy
When running Hydrodactyl behind a reverse proxy, such as Cloudflare's Flexible SSL
...
### Nginx
For Hydrodactyl to properly respond to an NGINX reverse proxy, the NGINX location config must contain the following lines:
```nginx
