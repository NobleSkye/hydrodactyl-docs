...
description: Learn how to setup a web server for your hydrodactyl panel
---

<Callout type="info">
  For Hydrodactyl-specific issues, join our Discord community at
  [https://discord.gg/mnTJVSSaKp](https://https://discord.gg/mnTJVSSaKp) for support.
</Callout>
...
<Tabs items={['Nginx with SSL', 'Apache with SSL', 'Caddy With Automatic SSL']}>
  <Tab value="Nginx with SSL">
    ...
    ```
  Now, you should paste the contents of the file below, replacing \<domain\> with your domain name being used in a file called `hydrodactyl.conf` and place the file in `/etc/nginx/sites-available/`, or — if on RHEL, Rocky Linux, or AlmaLinux, `/etc/nginx/conf.d/`.
...
server {
    ...
    access_log /var/log/nginx/hydrodactyl.app-access.log;
    ...
    location ~ \.php$ {
        ...
        fastcgi_pass unix:/run/php/php8.4-fpm.sock; # Change this if you are using a different php version [!code highlight]
...
sudo ln -s /etc/nginx/sites-available/hydrodactyl.conf /etc/nginx/sites-enabled/hydrodactyl.conf
...
```
  ...
  ```
  Now, you should paste the contents of the file below, replacing \<domain\> with your domain name being used in a file called hydrodactyl.conf and place the file in /etc/apache2/sites-available, or — if on RHEL, Rocky Linux, or AlmaLinux, /etc/httpd/conf.d/.

  Note: When using Apache, make sure you have the libapache2-mod-php8.4 package installed or else PHP will not display on your webserver.
```apache
<VirtualHost *:80>
    # Replace the example <domain> with your domain name or IP address
    # [!code highlight:1]
...
<VirtualHost *:443>
    ...
    # [!code highlight:2]
    ServerName <domain>
    DocumentRoot "/var/www/hydrodactyl/public"
    ...
    <Directory "/var/www/hydrodactyl/public">
        Require all granted
        AllowOverride all
    </Directory>
...
```
  ...
  sudo ln -s /etc/apache2/sites-available/hydrodactyl.conf /etc/apache2/sites-enabled/hydrodactyl.conf
  ...
  <domain> {
      ...
      log {
          output file /var/log/caddy/hydrodactyl.log {
              roll_size 100MiB
              roll_keep_for 7d
          }
          level INFO
