...
description: Configure Cloudflare as a DNS provider for Hydrodactyl subdomains.
---
...
- A Hydrodactyl panel admin account
...
1. Scope the token to the exact zone you want Hydrodactyl to manage:
...
<Callout type="info">
  Hydrodactyl stores the zone ID directly, so `Zone Read` is not required for the
...
Before adding the domain to Hydrodactyl, verify the token from any machine that can
...
If you receive an authentication error here, Hydrodactyl will fail too.

## Create The Domain In Hydrodactyl
...
1. Mark it as **Default** if you want Hydrodactyl to generate subdomains for new
   supported servers automatically.

## Configure The Node And Allocations

Hydrodactyl builds subdomain DNS records from the server's **primary allocation**.
...
<Callout type="warn">
  ...
  Hydrodactyl can create DNS records that point to the wrong address.
</Callout>

## Record Behavior

Hydrodactyl creates different records depending on the egg feature and port:
...
See [Egg Features](/docs/hydrodactyl/config/egg-features) for the supported
...
- If **Domain by Allocation Alias** is enabled, make sure the allocation has a
  public `ip_alias`
- If the alias is blank, Hydrodactyl may publish the allocation's internal IP
