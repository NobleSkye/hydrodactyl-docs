...
1. Initialize the Docker image: `pnpm build:docker:dev`
   ...
   - Configures the image for Hydrodactyl services
...
1. Access Hydrodactyl from http://localhost:3000/, and login using the default credentials of `dev@pyro.host` and `dev`
...
You can try searching these errors in the [Pterodactyl Discord server](https://discord.gg/pterodactyl) as someone has likely already found a solution. **However, DO NOT ask for support in the Pterodactyl Discord** as they do not support Hydrodactyl and _will not_ help you.

If you need to ask for help, ask in our [Hydrodactyl Discord server](https://https://discord.gg/mnTJVSSaKp).

### Ports in use

Try killing any processes that could be using Hydrodactyl's ports, i.e. 8080, 2022, and 3000 by default, then run `pnpm compose` again. However, if that won't work, try the below:
