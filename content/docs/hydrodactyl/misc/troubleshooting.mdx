...
description: Common issues and their solutions for Hydrodactyl
---
...
1. Check Elytra connection:
   ```bash
   docker exec -it hydrodactyl-elytra elytra debug
   ```
...
3. Check Docker container status:
   ```bash
   docker exec -it hydrodactyl-elytra docker ps -a
   ```
...
4. Review backup logs:
   ```bash
   docker exec -it hydrodactyl-elytra tail -f /var/log/elytra/backup.log
   ```
...
2. Monitor Elytra logs:
   ```bash
   docker exec -it hydrodactyl-elytra tail -f /var/log/elytra/elytra.log
   ```
...
### Discord Community
Join the [Hydrodactyl Discord](https://https://discord.gg/mnTJVSSaKp) for community support.

### GitHub Issues
Report bugs and feature requests at the [GitHub repository](https://github.com/blueprintframework/hydrodactyl/issues).
